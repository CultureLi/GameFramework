...
namespace Framework
{
    ...
    /// 打开界面时加载依赖资源事件。
    /// </summary>
    public sealed class OpenUIFormDependencyAsset : EventBase
    {
        /// <summary>
        /// 初始化打开界面时加载依赖资源事件的新实例。
        /// </summary>
        public OpenUIFormDependencyAsset()
        {
            SerialId = 0;
        ...
        public static OpenUIFormDependencyAsset Create(int serialId, string uiFormAssetName, string uiGroupName, bool pauseCoveredUIForm, string dependencyAssetName, int loadedCount, int totalCount, object userData)
        {
            OpenUIFormDependencyAsset openUIFormDependencyAssetEventArgs = ReferencePool.Acquire<OpenUIFormDependencyAsset>();
