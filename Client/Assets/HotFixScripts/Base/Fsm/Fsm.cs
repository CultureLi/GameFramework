using GameMain.Runtime.Base.Variable;
    ...
    public sealed class Fsm<T> : FsmBase, IReference, IFsm<T> where T : class
    {
        ...
        public static Fsm<T> Create(string name, T owner, params FsmState<T>[] states)
        {
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                {
                    throw new Exception($"FSM '{typeof(T)}' state '{stateType.FullName}' is already exist.");
                }
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            ...
            if (state == null)
            {
                throw new Exception($"FSM '{typeof(T)}' can not start state '{typeof(TState).FullName}' which is not exist.");
            }
        ...
        public void Start(Type stateType)
        {
            ...
            if (state == null)
            {
                throw new Exception($"FSM '{typeof(T)}' can not start state '{stateType.FullName}' which is not exist.");
            }
