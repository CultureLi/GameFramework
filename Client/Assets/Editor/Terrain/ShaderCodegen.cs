...
{
    public static class ShaderCodegen
    {
        public const string ShaderFolder = "Assets/BundleRes/Shader/Scene";
        public const string FoliageShaderPath = ShaderFolder + "/Foliage.shader";
        public const string BuildingShaderPath = ShaderFolder + "/SimpleBuilding.shader";
        public const string FoliageShaderName = "BundleRes/Foliage";
        public const string BuildingShaderName = "BundleRes/SimpleBuilding";

        public static void EnsureShaders(bool forceRegen)
        {
            Directory.CreateDirectory(ShaderFolder);
            EnsureFile(FoliageShaderPath, FoliageSource, forceRegen);
            EnsureFile(BuildingShaderPath, BuildingSource, forceRegen);
            AssetDatabase.Refresh();
        }

        static void EnsureFile(string path, string content, bool forceRegen)
        {
            if (!forceRegen && File.Exists(path)) return;
            File.WriteAllText(path, content);
            AssetDatabase.ImportAsset(path, ImportAssetOptions.ForceUpdate);
        }

        // --- Foliage: URP alpha-clipped double-sided with vertex wind wave ---
        const string FoliageSource = @"Shader ""BundleRes/Foliage""
{
    Properties
    {
...
";
        ...
        // --- Simple opaque building: URP Lit-lite ---
        const string BuildingSource = @"Shader ""BundleRes/SimpleBuilding""
{
    Properties
    {
        _BaseMap (""Base Map"", 2D) = ""white"" {}
